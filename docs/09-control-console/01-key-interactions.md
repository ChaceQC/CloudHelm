...
5. Release / Deploy Agent 生成 ReleasePlan 后，第二道 deployment approval
   ...
   `release_plan_sha256`。
6. 第二道审批事务提交后由 Ops Hub worker 自动继续，展示 Deployment
   Controller 与 Remote Agent operation、Compose 步骤、digest 复核、健康检查
   和最终 DeploymentResult；`run-next` 只用于调试/人工恢复。
...
## M9 Desktop 与用户权限交互（规划）

- 首次启动配置 HTTPS Ops Hub profile，不再只依赖编译时 API URL。
- Login/logout、device/session、用户邀请、启用/禁用和 role binding 管理。
- 新 Desktop 在本机生成 Ed25519 keypair，通过 login challenge 登记 public key；
  My Devices 页面可为 Local Runtime 创建 5 分钟内单次消费的 pairing challenge，
  并展示发起 Desktop、状态、过期与撤销结果。
- 页面、按钮、快捷键和批量动作按 effective permissions 与资源
  `allowed_actions` 隐藏/禁用。
- Developer、Reviewer、Operator、Deployment Approver、Auditor、Viewer
  展示不同功能；System Owner 也不能绕过自批门禁。
- API 返回 401 时刷新 session，403 时刷新 permission/capability。
- Environment 角色只在 Deployment/Approval 页面看到父 Project、关联 Task 和
  CI/安全结论的最小脱敏摘要，不显示或跳转到无 project scope 的独立详情。
- role binding 变化后按 sequence 事件刷新权限并清除无权缓存。
- Desktop 离线时只保存草稿；审批、部署、取消、回滚等高风险 intent 重连后
  必须重新认证、检查资源版本并由用户确认。
