...
- M7 部署 MVP：一台常在线 Ubuntu/Linux Ops Hub + 一个或多个受管
  staging/demo target。all-in-one demo 可同机，但 Ops Hub、Remote Agent 和业务
  项目必须隔离。
- M8 观测闭环：在 M7 部署结果上接入 Prometheus、Loki、Alertmanager 和 SRE
  分析。
- M9 Desktop/RBAC：Windows/Linux Tauri 安装包、SQLite/credential store、
  用户/session/device、scoped RBAC 和离线同步。
...
|能力|技术选型|说明|
|---|---|---|
|Ops Hub|Linux + Docker Compose/systemd|Platform API、Agents、Workflow、PostgreSQL/Redis 常在线；Desktop 可退出|
|用户与权限|User/Session/Device + scoped RBAC|不同 Desktop 用户按 system/project/environment scope 获得能力|
|远端环境|一台或少量 Ubuntu/Linux target|作为 staging / demo 业务运行环境|
...
|项目兼容|standalone Compose + versioned CloudHelm manifest|项目可脱离 Ops Hub 独立运行；受管时使用通用 renderer|
...
每套中心设施先执行一次 Ops Hub installation/bootstrap；每台受管 Linux 目标再
独立执行 Remote Target / Environment bootstrap，只安装 Docker/Compose、Remote
Agent、采集器和 machine credential，并注册到既有 Ops Hub。普通项目发布不得
重复执行任一 bootstrap。服务端已接受的流程由 Workflow Engine 持续推进，不依赖
Desktop 反复调用 `run-next`。
