...
M6 execution recipe 当前使用 `schema_version=1.1`；相较 1.0，
`acceptance_evidence[].testcase_names` 为必填稳定 pytest 函数名列表，并须
精确覆盖 RequirementSpec 的全部 AC。
...
保存 TestReport 与 JUnit Artifact。Tester 读取真实 XML，按 recipe 声明的
testcase 名逐 AC 映射；缺失或 skipped 为 `not_covered`，失败为 `failed`。
退出码、JUnit 统计、逐 AC 映射或报告解析任一不一致都不会标记完整通过，并
回到 Implementing；测试基础设施错误则暂停在 Testing。
...
- `changed_files` 与 `diff_paths` 必须非空、无重复、精确一致，git.diff 返回
  的 `paths` / `changed_files` 也必须相同。
- Reviewer 使用完整、未截断的持久化安全投影 diff，并核对每个
  created/updated/deleted 文件头；原始 patch bytes/SHA 只进入 Artifact 和
  Git 最终门禁。
- 受控 auth/profile recipe 必须包含约定模块、测试文件和领域 marker。
...
- 上述恢复只覆盖进入应用异常处理或已经留下可判定幂等证据的失败。Platform API
  进程在 AgentRun/ToolCall 终态持久化前被强制终止时，当前没有
  lease、heartbeat 或 stale reclaim；active 记录需人工核验，M6 不承诺
  hard crash 自动恢复。
