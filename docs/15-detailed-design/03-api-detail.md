...
- 正式 API 运行在常在线 Linux Ops Hub；Desktop、Local Runtime 和 Remote Agent
  都不直连 PostgreSQL/Redis。
- M7/M8 正常 continuation 由 PostgreSQL WorkflowJob + dispatcher/worker 推进；
  `remote-deployment/run-next` 只用于调试、逐步演示或人工恢复。
- M9 增加 Auth/User/Device/Session/Role/Permission/RoleBinding、resource
  capabilities 和 project snapshot/sequence sync。
- 所有受保护 API 从认证上下文派生 user/device/session，不接受调用方自报
  `actor_id` 作为授权身份。
- 当前 M2-M6 `created_by`/`actor_id` 字段继续作为 `0.5.1` 兼容审计字段；M9
  DTO 应忽略或拒绝调用方覆盖，并写入 `*_user_id`。
...
`created_by` 是当前 `0.5.1` 兼容字段。M9 受保护接口不再接受调用方指定该值，
由 access token 对应的 `user_id` 写入 `tasks.created_by_user_id`，响应中的
legacy 字段仅作兼容安全投影。
...
`actor_id` 同样只属于当前 M4/M6 本地审计语义。M9 后由认证上下文写
`initiated_by_user_id`，scope/permission/资源状态在服务端重新校验。
...
M9 接入用户认证后，TechnicalDesign 响应增加
`last_modified_source`、`last_modified_by_user_id`、
`last_modified_by_agent_run_id` 和
`content_sha256`。人类提交/修改从认证上下文写 user；Agent 生成/重写写当前
AgentRun，并通过 `agent_runs.initiated_by_user_id` 解析人类发起者。设计审批必须
绑定当前 `technical_design_id + version + content_sha256`，reviewer 与当前版本
最后修改者或 AgentRun 发起者相同则返回 `403 self_approval_forbidden`。
hash 使用 `technical-design-content.v1` canonical object，覆盖 design type、
Markdown、OpenAPI、DB schema、Mermaid、risk level 和 version；source 与 user/
AgentRun 引用按 `user | agent_run | legacy_system` CHECK 约束。
...
- M7/M8 正式远端流程在审批事务提交后创建/唤醒 durable WorkflowJob，由 worker
  继续；Approval HTTP 事务本身仍不执行外部副作用。
- M9 从认证上下文写 `decided_by_user_id`，并同时校验 permission、scope、
  resource version 与服务端 provenance；自批返回 403。
- `approve_technical_design` 还必须重新读取当前 design id/version/content hash
  和修改者 provenance；任一漂移返回 `409 stale_approval`，最后修改者或当前
  AgentRun 的人类发起者不得决定该审批。
...
当前数据库事件使用 PascalCase `event_type`。目标 envelope 示例：

|event|payload|
|---|---|
|`TaskPhaseChanged`|task_id、from、to、reason|
|`AgentRunStarted`|agent_run_id、agent_type|
|`AgentRunCompleted`|agent_run_id、status、summary|
|`ToolCallStarted`|tool_call_id、tool_name、risk_level|
|`ToolCallSucceeded`|tool_call_id、status、summary|
|`ApprovalRequested`|approval_id、action、risk_level|
|`ArtifactCreated`|artifact_id、artifact_type|
|`DeploymentUpdated`|deployment_id、status|
|`ProjectAlertFired`|alert_id、severity|

当前 `0.5.1` 断线恢复：

- SSE 每次回放当前已有 task events、追加 heartbeat 后关闭。
- Web 控制台固定退避重连，并按 UUID event id 去重。
- 当前没有 sequence、`Last-Event-ID` 增量游标、project snapshot 或真正 live
  tail；CI/deployment/heartbeat 的持续事件流属于后续实现。

M9 Desktop 同步不继续依赖 UUID last_event_id，而使用：
...
`sequence` 单调但不保证连续；权限过滤空洞正常。游标超过保留期返回
`event_cursor_reset_required`。每个事件带 project/aggregate identity/version、
schema version 和认证 actor refs，Desktop 按 aggregate version 防止旧覆盖。

## 9. Deployment / Remote Ops API（M7 渐进实现）

M7-1 已实现 Environment、RemoteTarget 和 machine-auth heartbeat，并同步到
FastAPI/共享 OpenAPI；M7-2B1/B2 已实现 RepositoryBinding、release candidate、
第一道审批与 pending reconcile WorkflowJob；M7-2C 已实现服务端 durable
dispatcher/worker/reclaimer 和真实数据库 reconcile handler。CI、deployment、
service/log API 仍是后续 M7 契约，以
...
- M7-2 当前领域门禁可将兼容 actor 字符串规范化为 AgentRun UUID，与 PR creator
  比较并拒绝相同值；该比较不构成不可伪造用户身份。
- M9 接入认证后，AgentRun 保存 `initiated_by_user_id`，ReleaseCandidate 保存
  `requested_by_user_id`；approve/reject 只比较认证 user 与服务端 provenance，
  相同返回 `403 self_approval_forbidden`，不再使用调用方 actor 字符串授权。
- M7-2C 已由独立 Workflow Engine 使用同一 PostgreSQL reserve 入口扫描、投递和
  补投；Redis/Celery message 只携带 `workflow_job_id`，业务权威仍在数据库。
...
- 只用于开发调试、答辩逐步展示或人工恢复，可推进 publish candidate、workflow
  dispatch、CI reconcile、Release Agent、deployment approval、dispatch、health
  verification 或 Monitoring handoff 中的一个动作。
...
- 正常 Ops Hub 流程由 durable dispatcher/worker 自动领取下一个可执行 job；
  Desktop 退出不停止已经持久化且不需要新审批的工作。
...
- 权限：当前 `0.5.1` 本地演示入口默认可读且不构成真实认证边界；M9 后按
  system/project scope、`system.roles.read`/相应调试权限和服务端 resource
  policy 过滤。
