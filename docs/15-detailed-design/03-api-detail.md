...
第一道审批与 pending reconcile WorkflowJob。CI、deployment、service/log API
仍是后续 M7 契约，以
...
- M7-2B2 提交后只保留 PostgreSQL pending job；尚无运行时 dispatcher 或 broker
  publish。M7-2C 周期 dispatcher 将使用同一 reserve 入口扫描和补投。
