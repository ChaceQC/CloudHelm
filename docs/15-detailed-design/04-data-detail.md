...
lease、撤销本次尚未执行的 attempt，并写 `WorkflowJobExecutionDeferred`；Task
resume 将 next enqueue 推进到
...
dispatcher 每轮先读取一次 `due_cutoff=clock_timestamp()`；due 条件必须同时服从
business retry 与 enqueue 时间，并统一复用该 cutoff：

```text
status=pending
attempt<max_attempts
next_retry_at IS NULL OR next_retry_at<=due_cutoff
next_enqueue_at<=due_cutoff
dispatch lease 为空或 expires_at<=due_cutoff
```

reserve 先按 Task -> WorkflowJob 加锁重验，为每条 reservation 写唯一
`dispatch_lease_owner` 和递增后的 enqueue attempt，提交后再 publish；success/
failure finalize 必须以
`id + status=pending + dispatch_lease_owner=reservation_owner +
expected_enqueue_attempt` 条件更新，避免旧 reservation 覆盖已经 claim 或重新
reserve 的 job。success finalize 必须清
`last_enqueue_error_code`。worker 同时访问
Task/job/resource 时固定按
`Task -> WorkflowJob -> ProjectRepositoryBinding -> ReleaseCandidate -> Approval`
加锁，并在全部资源锁后重新读取 `clock_timestamp()`、重验 lease/expiry；
