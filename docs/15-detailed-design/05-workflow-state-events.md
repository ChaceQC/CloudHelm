...
|WaitingDeployApproval|第二道审批已创建|Human|审批 Environment、RemoteTarget、CIRun、digest、ReleasePlan 和 request hash|approved / rejected|
...
M5 不实现审批通过后的自动恢复执行；M7 计划只为第二道 deployment approval
补充单次消费和显式 `run-next` 恢复，Approval API 事务本身仍不直接执行远端
副作用。
