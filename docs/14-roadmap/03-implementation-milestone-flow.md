...
- [ ] 完成 M7-2B2 ReleaseCandidate 原子创建、第一道 L2 Approval、reconcile
  WorkflowJob、201/200 幂等语义和 Task-first 并发门禁。
...
M7-2B1 RepositoryProfile/Binding PUT/GET 已经完成；已完成切片均通过对应真实
数据库、并发、契约和全量回归。

当前执行指针为 M7-2B2：实现 ReleaseCandidate 原子创建、第一道审批、
reconcile WorkflowJob 和 Task-first 并发门禁；随后进入 M7-2C durable Workflow
Engine。
