...
- [ ] 实现 M7-2D CIRun、Deployment、ServiceInstance 数据与 migration。
...
reconcile WorkflowJob、M7-2C durable Workflow Engine 已经完成；已完成切片均
通过对应真实数据库、并发、契约和回归。M7-2C 还通过 WSL 隔离 Redis restart
补投和真实 prefork hard-crash lease 回排。

当前执行指针为 M7-2D：实现 CIRun、Deployment、ServiceInstance 数据与 migration，
先建立真实 PostgreSQL/ORM/repository/contract 底座，再进入 candidate ref、
唯一 CI、ReleasePlan 和部署纵切。M7-2C 当前只注册
