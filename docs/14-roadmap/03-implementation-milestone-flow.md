...
reconcile WorkflowJob 已经完成；已完成切片均通过对应真实数据库、并发、契约和
全量回归。B2 Platform API 全量为 `308 passed, 1 skipped`。

当前执行指针为 M7-2C：实现 PostgreSQL 权威的 Redis + Celery durable Workflow
Engine，包括 dispatch、claim、lease、heartbeat、retry、Redis 补投和
side-effect-aware stale reclaim；首个生产 handler 为
