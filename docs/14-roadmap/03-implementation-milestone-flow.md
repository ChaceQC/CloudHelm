...
- [x] 完成 M7-2B1 server-controlled RepositoryProfile、Binding PUT/GET、
  Project 首次创建 mutex、幂等 PUT、repository identity 冲突映射、binding
  漂移失效、CORS PUT 和数据库并发回归。
- [x] 完成 M7-2B2 ReleaseCandidate 原子创建、第一道 L2 Approval、reconcile
  WorkflowJob、201/200 幂等语义、approve/reject freshness 与自批门禁，以及
  Task-first/Binding PUT/PR 创建并发回归。
- [x] 实现 M7-2C Redis + Celery durable Workflow Engine，包括 dispatch、claim、
  lease、heartbeat、补投和 side-effect-aware stale reclaim。
- [x] 实现 M7-2D CIRun、Deployment、ServiceInstance 数据与 migration，包括
  ORM、repository、严格共享 Record、L3 deployment Approval CHECK、SQL 三值/
  健康数据安全门禁、真实 `FOR UPDATE`、部分唯一并发、批次回滚和独立数据库
  migration 往返。
...
- [x] 实现严格空对象的 candidate POST，绑定最新版 PullRequestRecord、完整
  commit、binding snapshot/hash、candidate ref 和 request hash，并原子创建第一道
  L2 Approval；审批前禁止 push 和 CI。
- [ ] `remote-deployment/start` 只复用已有 approved Candidate，不重复创建第一道
  审批。
...
M7-1 `Environment + RemoteTarget + machine-auth heartbeat`、M7-2A 数据底座和
M7-2B1 RepositoryProfile/Binding PUT/GET、M7-2B2 Candidate/第一道审批/
reconcile WorkflowJob、M7-2C durable Workflow Engine、M7-2D CI/部署数据底座
已经完成；已完成切片均
通过对应真实数据库、并发、契约和回归。M7-2C 还通过 WSL 隔离 Redis restart
补投和真实 prefork hard-crash lease 回排。

当前执行指针进入 M7-2E：发布受控 candidate ref、回读精确 commit，并只触发一次
固定 Gitea `workflow_dispatch`，把真实 run identity 写入 CIRun。M7-2C 当前只注册
`release_candidate_reconcile -> release_candidate -> none`，不把 CI/deploy
handler 写成已交付。
