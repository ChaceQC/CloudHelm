...
|`image_ref`|TEXT|否|只含一个 `@digest` 分隔符的 OCI 引用；拒绝 URL scheme、userinfo 和多重 `@`。|
...
- `rollback_requested`：必须绑定第二道 L3 Approval、批准人、remote operation、
  started/finished、JSON object health summary、另一条历史 healthy Deployment
  和 rollback request Artifact；不得自引用。
...
- `health_summary_json` 为空或 JSON object；对象最多 32 个小写受控 key，value
...
- `failure_code` 最长 128；`failure_summary` 最长 2048，且只保存脱敏摘要。
...
`approve_release_candidate + release_candidate + L2` 组合继续有效。两类资源审批
action 都不得省略 `resource_type` 后借 SQL NULL 绕过组合门禁。
