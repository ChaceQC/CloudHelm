...
部署在远程主机或集群中的轻量 agent，负责业务项目心跳、命令执行、日志流、指标暴露和服务发现。

## 技术栈

systemd service + Docker Compose integration + metrics/log exporter。

## 上游依赖

Release / Deploy Agent、Deployment Controller、Remote Control Plane、Monitoring Collector。

## 主要输出

心跳、服务状态、部署结果、日志流、指标端点。
