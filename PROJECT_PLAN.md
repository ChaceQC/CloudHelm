...
- `release_ref_prefix`
- `status`
- 唯一约束：`project_id`、provider/repository id
...
- `target_ref`
- `request_hash`
- `status`: pending_approval/approved/published/stale/cancelled
- `approval_id`
- `published_remote_sha`
- `idempotency_key`
- `approved_at`、`published_at`
- 唯一约束：Task/idempotency key、repository/target ref
...
- 唯一约束：job type/idempotency key
- active/lease/retry 查询索引
