...
#### 7.4.1 Codex CLI 式 Agent 协作与沟通

项目开发过程中的 Agent 使用、委派和沟通参考 Codex CLI 的 thread / turn /
subagent 模型，并结合 CloudHelm 的 Tool Gateway 与审批边界落实：

- 主 Agent / root thread 负责保留用户目标、约束、关键决策和最终汇总；探索日志、
  测试输出、堆栈和中间分析不得无界堆入主线程。
- 只有边界清晰、可以独立验收的子任务才允许显式 spawn subagent。spawn 请求
  必须写明 role、objective、expected result、输入范围、是否等待全部结果和
  完成判定，不得用隐式角色切换冒充新线程。
- read-heavy 的检索、代码探索、测试分析、安全审查和文档核验可以并行；
...
- subagent 的权限不得高于父线程；工具调用仍需按 child role 经过 Tool Gateway、
  Policy、Approval、workspace allowlist、限流和审计，不得继承或复用未消费的
  密钥、凭据或过期审批。
- 运行中的后续消息必须区分“steer 当前 turn”和“queue 到下一 turn”；审批、
  反馈、暂停、取消和接管必须标明目标 thread / turn，不得静默改写已经提交的
  历史或把下一步要求混入当前执行。
- 当前会话原语的创建、完成、失败、取消和摘要回传必须在 conversation 与
  EventLog 中追踪，并保存父 AgentRun 绑定和 ToolCall 权限审计；后续接入真实
  child 执行时，还必须为每个 child 建立独立 AgentRun、工具记录、等待屏障和
  thread 可视化。
- 本节首先约束仓库开发过程中的 Agent 协作。CloudHelm 产品侧 M1-M6 只实现
  root/child conversation 持久化、父运行绑定、深度/数量门禁、父子工具权限
  交集、摘要回传和 Task 取消级联等内部原语；真实 child 调度、wait-all、
  通用 steer/queue、独立 thread 管理 API/UI 和 workspace/worktree 调度器尚未
  交付，任何文档、测试或答辩材料都不得把协作规则写成已上线运行时能力。
- 参考资料和采用结论统一维护在
  `informations/m4-agent-context/codex-responses-context.md`；如 Codex CLI
  官方行为变化，应先更新资料归档和契约文档，再调整实现。
...
- 软件测试必须同时符合黑盒测试和白盒测试的基本流程；不能只凭“能启动”“接口有返回”就判定功能完成。
- 每个实现阶段开始前，应根据 `PROJECT_PLAN.md`、需求描述、接口文档、数据模型和验收矩阵列出测试对象、测试范围、测试类型、测试数据、通过标准和不测范围。
- 每个可验证功能至少包含：正常路径、边界条件、异常输入、权限/审批约束、状态流转、数据持久化、副作用事件和失败恢复场景。
- 测试用例应能追溯到需求、API 契约、数据库字段、事件类型、Agent/Tool 契约或验收标准；无法追溯的测试应说明目的。
- 测试执行后必须记录命令、环境、结果、失败原因、修复动作和剩余风险；记录位置优先为 `PROJECT_PROGRESS.md`，必要时补充到 `docs/13-testing/` 或阶段测试报告。
- 缺陷必须形成闭环：发现 -> 定位 -> 修复 -> 回归 -> 记录；不得只修改代码不复测。
- 自动化测试优先，但人工测试、接口手测、截图或日志核查也必须写清楚步骤、输入、预期和实际结果。

### 8.2 黑盒测试要求

黑盒测试从用户、控制台、API 调用方、Agent 调用方或运维人员视角验证外部行为，不依赖内部实现细节。

- 依据需求规格、API 文档、OpenAPI、页面交互文档和验收标准设计测试用例。
- 必须覆盖等价类、边界值、必填/缺失字段、非法枚举、非法状态流转、重复提交、分页/过滤、幂等性、超时和错误响应。
- API 黑盒测试应验证状态码、响应结构、错误码、`trace_id`、分页字段、权限要求和事件副作用。
- 前端黑盒测试应验证页面入口、表单校验、加载态、错误态、空状态、真实数据展示、刷新/重试和关键用户路径。
- 部署与运维黑盒测试应验证启动命令、健康检查、日志回传、远端状态、审批提示和失败提示。
- 黑盒测试不得依赖生产代码中的 mock、固定返回或隐藏开关；测试 mock/stub/fake 只能存在于测试代码或测试夹具中。

### 8.3 白盒测试要求

白盒测试从代码结构、分支逻辑、状态机、事务边界和安全策略视角验证内部实现。

- 依据源码、分层设计、状态机、数据库事务、policy、service、repository、workflow 和工具风险等级设计测试用例。
- 后端应覆盖 service、repository、schema 校验、异常分支、事务回滚、事件写入、状态迁移和权限/审批策略。
- 前端应覆盖复杂 hook、API client、数据转换、错误处理、条件渲染、关键组件状态和交互回调；简单纯展示组件可按风险选择快照或集成测试。
- Agent/Tool Gateway 后续实现时，应覆盖结构化输出解析、工具参数校验、风险等级判断、审批拦截、审计写入、超时、重试和失败恢复。
- 白盒测试应关注语句、分支、条件、路径和异常处理覆盖；核心 service/workflow/policy 不得只测成功路径。
- 对数据库、文件系统、网络、LLM、Docker、Git、SSH 等外部依赖，应使用测试数据库、临时目录、测试夹具或隔离环境验证，不得污染真实环境。

### 8.4 提交与合并前测试门禁

- 提交到 `dev` 前，必须执行与改动范围匹配的黑盒和白盒测试；只改文档时至少执行文档结构、链接或 diff 检查，并说明未运行代码测试的原因。
- 从 `dev` 合并到 `main` 前，必须执行当前阶段定义的完整验证命令，并确认黑盒、白盒测试结果均已记录。
- 如果某项黑盒或白盒测试暂时无法执行，必须在 `PROJECT_PROGRESS.md` 写明原因、影响范围、替代验证和后续补测计划；对应能力不得标记为完全完成。
- 测试失败时不得提交“完成”类 commit，不得在总排期中打钩；修复后必须执行回归测试。
