...
CloudHelm 是面向毕业设计演示的多 Agent DevOps 系统。本仓库当前已完成 **M2：数据模型、API 与事件底座**，在 M1 Monorepo 工程基线之上接入 PostgreSQL、Alembic、SQLAlchemy 分层和真实 API 数据流。

> 当前版本：`0.2.0`
> 当前范围：Project、Task、Requirement、Technical Design、AgentRun、ToolCall、Approval 和 Event Timeline 的真实数据库 API。M2 不实现 Agent 自动执行、Tool Gateway 真实工具执行、远端部署或监控业务逻辑。
...
OpenAPI 覆盖 M2 已实现接口；事件 schema 增加 M2 真实事件类型；工具风险等级 schema 明确 M2 只记录 ToolCall 和 ApprovalRequest，真实 Tool Gateway 执行留到后续阶段。
...
M2 不实现 Agent 自动生成、LangGraph 编排、Tool Gateway 真实工具执行、Git PR、远端部署、监控告警或完整桌面端壳。上述能力将在 M3 及后续里程碑按 `PROJECT_PLAN.md` 和 `docs/14-roadmap/03-implementation-milestone-flow.md` 逐步实现。
