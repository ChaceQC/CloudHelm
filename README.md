...
**M1-M6**，并完成 M7 首个纵切
**Environment + RemoteTarget + machine-auth heartbeat**。当前已实现系统在已审批
...
> Remote Agent 可通过 HMAC 上报心跳。真实 Gitea CI、远端部署和监控仍未交付。
...
modules/
  platform-api/           # FastAPI 平台 API、M6 工作流与 M7-1 心跳入口
...
M7-1 不实现 Release / Deploy / SRE Agent、真实远端 PR、Gitea CI、远端
