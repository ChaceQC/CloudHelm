...
CloudHelm 是面向毕业设计演示的多 Agent DevOps 系统。本仓库当前已完成 **M5：Tool Gateway 与本地工具层**，在 M4 结构化编排基础上接入本地工具统一入口、风险等级、审批拦截、审计记录和控制台 ToolCall 展示。
...
M5 默认工具包括 `requirement.normalize`、`design.render_markdown`、`repo.*`、`sandbox.*`、`git.*` 和 `approval.request_remote_action`。其中 `approval.request_remote_action` 只用于验证 L3 审批拦截，不执行远端动作。
