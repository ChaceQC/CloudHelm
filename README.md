...
**M1-M6**，并完成 M7 首个纵切
**Environment + RemoteTarget + machine-auth heartbeat**。系统在已审批 DevelopmentPlan
...
M7-1 不实现 Release / Deploy / SRE Agent、真实远端 PR、Gitea CI、SSH 执行、
远端 Compose deployment、监控告警或完整桌面端壳。Sandbox 当前使用本地受控目录 + subprocess 超时，
