...
- [Celery 5.6.3（PyPI）](https://pypi.org/project/celery/5.6.3/)
- [redis-py 8.0.1（PyPI 最新稳定版）](https://pypi.org/project/redis/8.0.1/)
- [redis-py 6.4.0（CloudHelm 采用版）](https://pypi.org/project/redis/6.4.0/)
- [Kombu 5.6.2（PyPI）](https://pypi.org/project/kombu/5.6.2/)
...
- 2026-07-16 通过 PyPI 官方 JSON metadata 复核：Celery 最新稳定版为
  `5.6.3`，要求 Python `>=3.9`；redis-py 最新稳定版为 `8.0.1`，要求
  Python `>=3.10`。两者均覆盖 CloudHelm 当前 Python `>=3.12` 基线。
- Celery `5.6.3` 当前解析到 Kombu `5.6.2`；Kombu 的 `redis` extra 要求
  `redis!=4.5.5,!=5.0.2,<6.5,>=4.5.2`。因此 redis-py `8.0.1` 虽是最新
  稳定版，但不在该组合的兼容范围内；`6.4.0` 是 `<6.5` 范围内的最新稳定版。
...
- `modules/workflow-engine` 独立固定 `celery[redis]==5.6.3` 与
  `redis==6.4.0`，由模块自己的 `uv.lock` 固化完整传递依赖；Platform API
  不反向依赖该模块。redis-py `8.0.1` 暂不采用，待 Celery/Kombu 官方兼容
  上限提升且完整 broker 回归通过后再升级。
