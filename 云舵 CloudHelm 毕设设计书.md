...
> 文档版本：v0.2
> 最近修订：2026-07-16
> 关键词：云舵、CloudHelm、AI Agent、MCP、Tool Gateway、跨平台 Desktop、
> CloudHelm Ops Hub、Local Runtime、用户与分层权限、本地开发、Agent 化远程
> 部署、CI 构建流水线、实时监控、自动运维、人机协同、安全审计、独立项目交付
...
1. **CloudHelm Desktop**：Windows/Linux 可安装客户端，负责需求、审查、审批、
   diff、状态和监控展示；最终用户不安装 Docker、PostgreSQL、Redis 或 Python。
2. **Local Runtime**：随 Desktop 分发的本机 sidecar，负责 allowlist workspace、
   Git、测试和受控工具执行。
3. **CloudHelm Ops Hub**：持续运行的 Linux 控制面，承载 Platform API、
   Orchestrator、Agent Runtime、Tool Gateway、Workflow Engine、Deployment
   Controller、PostgreSQL、Redis、身份权限和审计。
4. **Remote Agent 与业务项目**：Remote Agent 管理受控部署；业务项目拥有独立
   源码、配置、数据库和生命周期，可脱离 CloudHelm 独立运行。

Ops Hub 支持多个用户通过 system/project/environment scoped RBAC 使用不同
Desktop 功能。Desktop 的页面/按钮门禁只改善体验，每个 API 请求都由服务端按
permission、scope、资源属性和职责分离重新鉴权。
...
14. 支持桌面端控制台，体验参考 Codex App：
   ...
   - 工具调用记录。
   - 审批按钮。
15. 支持 Windows NSIS setup `.exe`、安装后的 `CloudHelm.exe`，以及 Linux
    AppImage 和 `.deb` 安装包。
16. 支持用户、Desktop device/session、预置角色、permission 和
    system/project/environment scope，让不同权限用户在 Desktop 中看到并执行
    不同功能。
17. 支持 Desktop 离线/退出后，Ops Hub 继续执行已持久化且不需要新审批的 CI、
    部署、监控和 Agent 工作，并在重连后按 event sequence 补齐。
18. Agent 生成的业务项目必须提供独立源码、锁文件、README、测试、Dockerfile、
    standalone Compose、配置和 migration；移除 CloudHelm adapter 后仍可运行。
...
9. **可安装性**：Windows/Linux Desktop 在无 Docker/PostgreSQL/Redis 的干净环境
   可安装、升级和卸载。
10. **持续在线性**：Ops Hub 生命周期独立于 Desktop，App 退出或断网不停止远端
    工作流。
11. **身份安全性**：用户、Desktop/Local Runtime device 和 Remote Agent machine
    identity 分离；默认拒绝、最小权限并禁止自批。
12. **项目可移植性**：同一 commit 能通过 standalone 与 CloudHelm-managed 两条
    路径部署，业务项目不依赖 CloudHelm SDK 或平台数据库。
...
|Desktop 本地数据|SQLite + 独立 migration|只保存 server profile、UI 设置、草稿、缓存和事件 sequence|
|Desktop 凭据|OS credential store / Tauri Stronghold|保存 access/refresh token 与 Ed25519 device private key，不写 SQLite|
|本地执行|Tauri sidecar + `modules/local-runtime`|本机 workspace、Git、测试和受控工具；不直连远端数据库|
...
|用户与权限|User/Device/Session + scoped RBAC + domain policy|按 system/project/environment scope 控制 Desktop/API；System Owner 也不能绕过职责分离|
...
|Ops Hub 部署|Linux + Docker Compose/systemd|Platform、Agents、Workflow、PostgreSQL/Redis 常在线；Desktop 可退出|
|项目兼容契约|`cloudhelm.project.yaml` + `cloudhelm.env.schema.json`|可删除 adapter；Controller 使用固定 schema + 通用安全 renderer|
...
```text
Tauri + React + TypeScript + Desktop SQLite + OS credential store
...
cloudhelm.project.yaml + cloudhelm.env.schema.json + 通用安全 renderer
Semgrep + Trivy
Langfuse
Prometheus + Grafana
```
...
```mermaid
flowchart LR
    User["开发者 / Reviewer / 运维 / Auditor"]
    Desktop["CloudHelm Desktop\nTauri + React"]
    DesktopDB[("SQLite\nprofile / draft / cache / sequence")]
    Credential["OS Credential Store"]
    LocalRuntime["Local Runtime Sidecar\nworkspace / Git / test"]

    Ingress["HTTPS Ingress"]
    OpsHub["CloudHelm Ops Hub\nLinux always-on"]
    API["Platform API + Identity/RBAC"]
    ...
    DB[("PostgreSQL\n权威状态")]
    Redis[("Redis/Celery\n非权威投递")]
    GitCI["Gitea / CI Runner / OCI Registry"]
    Obs["M8 Prometheus / Loki / Alertmanager"]

    RemoteAgent["Remote Agent\nsystemd"]
    Project["Independent Business Project\nstandalone / managed Compose"]

    User --> Desktop
    Desktop --> DesktopDB
    Desktop --> Credential
    Desktop --> LocalRuntime
    Desktop -->|"REST / SSE"| Ingress
    LocalRuntime -->|"authenticated device channel"| Ingress
    Ingress --> OpsHub
    OpsHub --> API
    API --> DB
    API --> Orchestrator
...
|区域|运行内容|说明|
|---|---|---|
|本地桌面端|Tauri 控制台、SQLite 非权威缓存、OS credential store、需求/审批/状态 UI|Windows/Linux 可安装客户端；退出不停止远端工作；不直连 PostgreSQL、Redis 或 Remote Agent|
|本地 Agent 开发区|Local Runtime、Git worktree、受控 sandbox、项目模板、测试执行|只访问用户 allowlist workspace；本机离线时本地步骤暂停|
|规格与设计区|Requirement Spec、ADR、OpenAPI、数据库 schema、验收标准|保存开发者指导和 Agent 设计产物，作为后续实现、测试和审查的依据|
|常在线 Ops Hub|FastAPI、Orchestrator、Agent Runtime、Tool Gateway、Workflow Engine、PostgreSQL、Redis、Identity/RBAC|固定部署在持续运行的 Linux 主机；本地 Compose 只供 contributor development/all-in-one demo|
...
10. **Desktop 与常驻控制面分离**：Desktop 是客户端，Ops Hub 是权威宿主；
    Desktop 关闭不停止已经持久化且无需新审批的远端流程。
11. **用户授权与 Agent 权限分离**：人类用户按 scoped RBAC + resource/domain
    policy，Agent/Tool 按 role/lineage/Tool Gateway policy；两层都必须通过。
12. **项目必须可剥离**：业务项目不 import CloudHelm SDK、不连接平台数据库；
    删除 adapter 后仍能独立构建、测试、部署和运行。
13. **正常流程服务端推进**：API 事务写业务状态、EventLog 和 WorkflowJob 后由
    worker 继续；`run-next` 只用于调试、答辩逐步展示和人工恢复。
...
|目录|职责|
|---|---|
|`apps/control-console`|Windows/Linux Desktop，负责 server profile、登录、需求、审查、diff、审批、远程状态和按权限展示；SQLite 仅保存非权威本地数据|
|`modules/local-runtime`|随 Desktop 分发的本机 sidecar，负责 allowlist workspace、Git、测试和受控工具执行|
...
|`modules/deployment-controller`|校验项目/环境 schema，使用通用安全 renderer 生成受管 Compose，管理部署、健康检查和回滚候选|
...
6. 审批事务提交后由 durable WorkflowJob/worker 自动继续；Deployment Controller
   校验 `cloudhelm.project.yaml`、`cloudhelm.env.schema.json` 和 manifest hash，
   使用固定 schema + 通用安全 renderer 生成 fixed-digest Compose；secret 只通过
   远端 env profile / credential store 引用。调试 `run-next` 只用于人工恢复。
...
|---|---|
|User|Ops Hub 登录用户|
|Device|Desktop 或 Local Runtime device identity；与 Remote Agent machine identity 分离|
...
|Role / Permission / RoleBinding|预置角色、精确权限和 system/project/environment scoped binding|
|Project|接入的平台项目或仓库|
|Task|一次用户目标或自动触发任务|
...
1. Ops Hub PostgreSQL：平台任务、Agent、审批、事件、WorkflowJob、用户/RBAC、
   部署和审计的唯一权威。
2. Desktop SQLite：server profile、UI 设置、草稿、缓存和 event sequence；可重建，
   不复制 PostgreSQL 表，凭据不写入。
3. 业务项目数据库：由业务项目自行选择和迁移，不复用 CloudHelm 平台
...
M9 为 Design approval 职责分离增加当前版本的
`last_modified_source`、`last_modified_by_user_id`、
`last_modified_by_agent_run_id` 和 `content_sha256`。source 约束 user/AgentRun
引用恰有一项非空，历史 migration 可使用 `legacy_system`。content hash 使用统一
stable JSON 覆盖设计类型、Markdown、OpenAPI、DB schema、Mermaid、risk level 和
version；审批绑定 `technical_design_id + version + content_sha256`，并比较
reviewer 与最后修改者或对应 AgentRun 的人类发起者。
...
M9 为 Desktop 离线同步增加 `sequence`、`stream_kind`、`project_id`、
`aggregate_type`、`aggregate_id`、`aggregate_version`、`schema_version`、认证
user/device/session actor 引用和 `subject_user_id`。`sequence` 在单个 Ops Hub
内单调递增，SSE `id` 使用 sequence；用户控制流按 subject user 读取，不能把执行
撤权的 actor 当作被撤权用户。

#### users / devices / user_sessions / roles / permissions / role_bindings（M9）

用户与权限表支持：

- 一次性 bootstrap 与第一个 `system_owner`。
- 邀请 token hash、Argon2id password hash。
- Desktop/Local Runtime device lifecycle，以及只保存 one-time code hash、TTL、消费
  状态和发起 Desktop device 的 Local Runtime pairing challenge。
- device 只保存 Ed25519 public key/fingerprint/version，private key 进入本机
  credential store。
- fingerprint 对解码后的 32-byte public key 求 SHA-256；首次 version 固定为 1，
  M9 不提供原地 key rotation。
- Desktop session 使用 refresh token rotation history/token family；Local Runtime
  只使用短期 device-bound access token。
- 预置 role、permission、多对多 role_permissions。
...
snapshot 返回 `as_of_sequence`；SSE `id` 使用 sequence。游标超过保留期时返回
`event_cursor_reset_required`，Desktop 重新构建本地 read model。权限过滤造成的
sequence 空洞正常。
...
### 12.9 Auth / User / Permission API（M9）
...
所有受保护 API 从 access token 派生 user/device/session，按 permission、scope、
resource version 和职责分离重新鉴权。Desktop 可以隐藏按钮，但直接构造 HTTP
请求仍必须被服务端 401/403 拒绝。
...
10. 完成后展示：
   ...
   - 安全扫描结果。
   - Agent 总结。
11. Desktop 根据 effective permissions 和资源 `allowed_actions` 控制路由、按钮、
    快捷键和批量操作；服务端 403 后刷新权限并清理无权缓存。
12. Desktop 退出或断网期间，Ops Hub 中无需新审批的工作继续；重连后通过
    snapshot + event sequence + SSE 补齐。
...
|用户权限|User/Device/Session + scoped RBAC + resource/domain guard；Desktop UI 不是安全边界|
|Agent 权限|按 Agent role、conversation lineage、Tool Gateway policy 发放最小工具权限|
|成本|按 project / task / agent 设置 token 和资源预算|

用户授权和 Agent 工具权限是两层独立门禁。System Owner 管理用户和 binding，
但不能批准自己发起或实现的 release/deployment；用户 token 与 Remote Agent
machine credential 不复用。
...
以下 Docker Compose 只用于仓库贡献者开发，不是 Desktop 最终用户安装方式：
...
正式 Desktop 由 M9/M10 交付：

```text
Windows: CloudHelm_<version>_x64-setup.exe + CloudHelm.exe
Linux:   CloudHelm_<version>_amd64.AppImage + cloudhelm_<version>_amd64.deb
```

安装包包含 Tauri、React assets、Local Runtime sidecar、Desktop SQLite migration
和更新元数据，不包含 PostgreSQL、Redis、Docker、Remote Agent 或 Ops Hub secret。

### 16.2 远端业务项目演示部署

答辩时准备一台持续运行的 Linux Ops Hub/远端演示主机，可以是云服务器、局域网
Linux 虚拟机或另一台主机。
...
Remote Linux Server / VPS
  ...
  - M8 cloudhelm-observability Compose project
```
...
5. 用户审批绑定 ReleasePlan、digest、Environment 和 RemoteTarget 的
   deployment approval。
6. Deployment Controller 校验 `cloudhelm.project.yaml`、
   `cloudhelm.env.schema.json` 和 manifest hash，使用固定 schema + 通用安全
   renderer 生成 fixed-digest Compose；secret 仅引用远端预配置 env profile /
   credential store。
...
答辩演示采用“可安装 Desktop + 常在线 Linux Host”。Desktop 无 Docker/
PostgreSQL/Redis 前置依赖；同机 demo 仍保持各 Compose project 生命周期隔离：

```text
Windows/Linux Desktop
  - CloudHelm Desktop
  - Local Runtime
  - SQLite + OS credential store
...
Ops Hub installation/bootstrap 以 CloudHelm 中心设施实例为边界执行一次，安装/
验证 TLS ingress、Platform/Workers、PostgreSQL/Redis、Gitea/registry、服务凭据、
持久卷和备份。M7 沿用当前受控网络/认证边界，不创建真实用户、device 或 session；
一次性 token 和首个 `system_owner` 属于 M9 identity bootstrap。每台受管 Linux
目标再独立执行 Remote Target /
Environment bootstrap，只安装 Docker/Compose、Remote Agent、采集器和 machine
credential，并注册到既有 Ops Hub。`demo-all-in-one` 可以把两条安装链放在同一
主机执行，但 manifest、credential、升级和卸载生命周期仍分离。普通业务项目发布
不重复执行任一 bootstrap，项目卸载也不得删除 Ops Hub、Remote Agent 或审计数据。
...
冻结设计书、MVP 裁剪线、模块/API/数据/测试契约、Git 与进度流程。

### M1 Monorepo 与最小工程

完成 FastAPI `/health`、React/Vite 控制台、共享契约和仓库基础结构。

### M2 数据模型、API 与事件底座

完成 PostgreSQL/Alembic、Project/Task/Requirement/Design/AgentRun/ToolCall/
Approval/EventLog 与基础 REST/SSE。

### M3 控制台主流程

完成项目/任务/详情/Timeline/审批的真实 API 调用、请求竞态和 SSE 去重。

### M4 Agent 编排与结构化输出

完成 Requirement/Architect/Planner、显式状态机、Task root conversation、多轮
provider/tool item 和设计/计划审批；真实 child 调度继续保持未交付边界。

### M5 Tool Gateway 与本地工具

完成工具 registry、schema、risk、approval、audit、workspace allowlist、
Repo/Sandbox/Test/Security/Git 工具。

### M6 本地代码、测试与 PR 等价闭环

完成固定 sample repo 的 Scaffold/Coder/Tester/Reviewer/Security、真实 diff、
pytest/JUnit、Bandit/pip-audit、branch/commit/format patch 与本地 PR record。

### M7 Ops Hub 常驻控制面、CI 与远端部署

完成 Linux Ops Hub、durable WorkflowJob/worker、Environment/RemoteTarget、
真实 Gitea CI/registry、不可变 OCI digest、两道审批、通用项目契约/renderer、
Remote Agent Compose deployment 和 Linux staging E2E。Desktop 退出后无需新审批
的流程继续。

### M8 监控、告警与 SRE

完成 Prometheus/Loki/Alertmanager、Monitoring Collector、ProjectAlert/
Incident、SRE 分析与 runbook proposal。

### M9 Desktop、用户/RBAC 与安全产品化

完成 Tauri、Local Runtime、运行时 server profile、Desktop SQLite、OS
credential store、snapshot/sequence sync、用户/device/session、角色、permission、
system/project/environment scope、职责分离和权限化 UI。

### M10 跨平台发行与最终验收

完成 Windows NSIS setup `.exe`/`CloudHelm.exe`、Linux AppImage/`.deb`、签名/
checksum、安装/升级/卸载、Ops Hub bootstrap/backup/restore/uninstall、
standalone/managed 双路径、答辩材料和 `v1.0.0`。
...
10. **Desktop 与 Ops Hub 分离**：可安装客户端可以退出，常驻控制面仍持续推进
    工作流和审计。
11. **用户分层权限**：role + scope + resource attributes + 职责分离同时控制
    Desktop 体验和服务端 API；Environment binding 不获得独立 Project/Task/CI
    读取，只接收当前部署直接关联的最小脱敏摘要。
12. **业务项目可剥离**：同一项目既能受 CloudHelm 管理，也能删除 adapter 后
    独立部署运行。
...
|Desktop 与服务端边界混淆|把 PostgreSQL/Docker 打进 Desktop 会导致安装复杂且 App 退出即停止工作|Desktop 只保存 SQLite 非权威数据；PostgreSQL/Redis 固定在 Linux Ops Hub|
|权限误配或自批|角色过大或只靠前端隐藏按钮可能越权|服务端默认拒绝、scope/resource 重验、`allowed_actions` 和职责分离；System Owner 也不绕过|
|业务项目被平台绑死|项目依赖 CloudHelm SDK/数据库后难以独立交付|Project Core + 可删除 Adapter + 通用 renderer + standalone/managed 双路径验收|
...
1. Windows NSIS setup `.exe`、安装后的 `CloudHelm.exe`、Linux AppImage 和
   `.deb`。
2. 一个可运行的 Tauri + React Desktop、Local Runtime、SQLite/credential
   store 和离线同步能力。
3. 一个常在线 Linux Ops Hub，包含 FastAPI、Workflow、PostgreSQL/Redis、
   安装升级和备份恢复。
4. 用户/device/session、预置角色、scoped RBAC、服务端授权与 Desktop 功能门禁。
5. 一个 Agent 编排器。
...
10. 一个项目脚手架 / 模块生成能力。
11. 一个 Remote Agent。
12. 一个使用通用项目契约 renderer 的 Deployment Controller。
13. 一个远端业务项目部署演示环境。
14. 一个远端业务项目监控与告警演示。
15. 一个可删除 CloudHelm Adapter、可 standalone/managed 双路径交付的示例项目。
...
```text
开发者创建功能开发目标
  -> 使用 project_developer 登录 Desktop
  ...
  -> deployment_approver 独立审批 deployment
  -> 关闭 Desktop，Ops Hub 继续执行
...
22. Windows/Linux Desktop 在无 Docker/PostgreSQL/Redis 的干净环境可安装运行。
23. 不同角色在 Desktop 中拥有不同页面和动作，直接 HTTP 越权返回 401/403。
24. System Owner、Developer、Reviewer、Operator 和 Approver 均不能绕过自批门禁。
25. Desktop 退出后，已持久化且无需新审批的 CI、部署和监控继续执行。
26. Desktop 重连后通过 snapshot + event sequence + SSE 补齐，无丢失、重复或旧覆盖。
27. 删除 `cloudhelm.project.yaml` 与 `cloudhelm.env.schema.json` 后，业务项目仍能
    standalone 构建、测试、部署和运行。
28. 同一 commit 的 standalone 与 managed 部署核心行为一致，Ops Hub、Remote
    Agent、观测栈和业务项目生命周期隔离。
...
13. 接入企业 OIDC/SAML、organization/tenant 和外部目录同步。
14. 增加 HA Ops Hub、托管 PostgreSQL/Redis、Kubernetes 控制面和多区域灾备。

---

## 24. 一句话总结

本毕设的核心不是“让开发者手写代码”，也不是“让大模型随便写代码”，而是构建
一个 **以跨平台 Desktop 交互、Linux Ops Hub 常驻、用户分层权限和可剥离业务项目
为边界，由开发者指导 Agents 完成开发、部署与实时运维的一体化 DevOps Agent
Platform**。
