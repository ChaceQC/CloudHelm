...
def list_tool_gateway_tools(request: Request, db: DbSession) -> PageResponse[ToolDeclarationRead]:
    """读取当前注册的本地工具声明。"""

    return ToolGatewayService(db, request.app.state.tool_gateway).list_tools()
...
def call_tool_gateway(task_id: UUID, payload: ToolGatewayCallCreate, request: Request, db: DbSession) -> ToolCallRead:
    """执行低风险工具；L3/L4 或策略要求审批时只创建审批请求。"""

    return ToolGatewayService(db, request.app.state.tool_gateway).call_tool(task_id, payload)
