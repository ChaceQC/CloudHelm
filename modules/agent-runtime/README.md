...
CloudHelm Agent Runtime `0.4.1` 提供 Requirement / Architect / Planner /
...
- Agent Runtime 提供参考 Codex CLI 的 subagent prompt、过滤 fork context、
  父子 metadata 和有界脱敏通知契约；默认深度 1、active child 上限 6、父运行
  绑定和父子工具交集由 Platform API / Tool Gateway 执行。read-heavy 并行和
  共享写状态串行/隔离属于仓库开发协作规则；M1-M6 尚未接入真实 child 执行、
  wait-all、steer/queue 或 thread 管理 UI。
...
- `prompts/subagent.md` v3：child conversation 的 fresh/full-history、父级与
  child 工具交集、生命周期、4000 字符摘要上限和最终通知边界。
