...
- `scaffold.prepare_workspace`：复制受控 fixture、初始化 baseline Git，并用
  Scaffold marker 校验幂等复用身份。
- `sandbox.run_command`、`sandbox.collect_artifact`：使用本地受控目录、
  命令数组、环境注入防护与进程树超时。
- `test.run_pytest`：执行真实 pytest、生成并解析 JUnit XML。
- `security.run_bandit`、`security.run_pip_audit`：返回结构化 findings，并保留
  pip-audit 明确跳过的依赖。
- `git.status`、`git.diff`、`git.create_branch`、`git.commit`、
  `git.format_patch`：只操作受控 Git 仓库根目录，不 push、不 rebase、不 tag。
...
quota 已在 M6 评估后保留为后续增强；M7 接入远端 staging/demo 前再次评估。
当前调用频率限流为单实例内存实现；多 worker
